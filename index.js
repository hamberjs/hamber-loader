...
const { compile, preprocess } = require('hamber');
...
function normalize(compiled) {
	// hamber.compile signature changed in 1.60 — this avoids
...
function deprecatePreprocessOptions(options) {
	...
	['markup', 'style', 'script'].forEach(kind => {
		if (options[kind]) {
			if (!warned[kind]) {
				console.warn(`[hamber-loader] DEPRECATION: options.${kind} is now options.preprocess.${kind}`);
				warned[kind] = true;
			}
...
module.exports = function(source, map) {
	...
	if (!('shared' in options)) options.shared = options.format === 'es' && 'hamber/shared.js';
